...
FrostMessage m(2u, FROST_MESSAGE::KEEP_ALIVE, move(pk));

cex::stream<std::deque<uint8_t>> s;

uint8_t etalon[] = {0, 1, 0, 2, 0, 0, 0, 0, 0x11, 0x22, 0x33, 0x44, 0x55, 0x66, 0x77, 0x88, 0x99, 0x00, 0xaa, 0xbb, 0xcc, 0xdd, 0xee, 0xff, 0x11, 0x22, 0x33, 0x44, 0x55, 0x66, 0x77, 0x88, 0x99, 0x00, 0xaa, 0xbb, 0xcc, 0xdd, 0xee, 0xff};

REQUIRE_NOTHROW(m.Serialize(s));
//m.Serialize(s);
...
frost_message_ptr res;
REQUIRE_NOTHROW(res = Unserialize(w.Secp256k1Context(), s));

FrostMessage *m1 = nullptr;
REQUIRE((m1 = dynamic_cast<FrostMessage *>(res.get())));

CHECK((m.sequence == m1->sequence));
...
NonceCommitments m(3u, move(pk));
m.nonce_commitments.emplace_back(move(pubnonce));

cex::stream<std::deque<uint8_t>> s;
REQUIRE_NOTHROW(m.Serialize(w.Secp256k1Context(), s));
...
do {
GetStrongRandBytes(sk3);
} while (!secp256k1_ec_seckey_verify(w.Secp256k1Context(), sk3.data()));

secp256k1_pubkey pk1, pk2, pk3;
if (!secp256k1_ec_pubkey_create(w.Secp256k1Context(), &pk1, sk1.data())) {
throw WrongKeyError();
}
if (!secp256k1_ec_pubkey_create(w.Secp256k1Context(), &pk2, sk2.data())) {
throw WrongKeyError();
}
if (!secp256k1_ec_pubkey_create(w.Secp256k1Context(), &pk3, sk3.data())) {
throw WrongKeyError();
}

l15::xonly_pubkey pk = ParseHex("11223344556677889900AABBCCDDEEFF11223344556677889900AABBCCDDEEFF");

KeyShareCommitment m(4u, move(pk));
m.share_commitment.emplace_back(move(pk1));
m.share_commitment.emplace_back(move(pk2));
m.share_commitment.emplace_back(move(pk3));
...
KeyShareCommitment m(5u, move(pk0));
...
{
scalar opid(ParseHex("00112233445566778899AABBCCDDEEFF00112233445566778899AABBCCDDEEFF"));
l15::xonly_pubkey pk = ParseHex("11223344556677889900AABBCCDDEEFF11223344556677889900AABBCCDDEEFF");
SignatureCommitment m(7u, move(pk), opid, opid);

cex::stream<std::deque<uint8_t>> s;

uint8_t etalon[] = {0, 1, 0, 7, 0, 0, 0, 4,
                    0x11, 0x22, 0x33, 0x44, 0x55, 0x66, 0x77, 0x88, 0x99, 0x00, 0xaa, 0xbb, 0xcc, 0xdd, 0xee, 0xff,
                    0x11, 0x22, 0x33, 0x44, 0x55, 0x66, 0x77, 0x88, 0x99, 0x00, 0xaa, 0xbb, 0xcc, 0xdd, 0xee, 0xff,
