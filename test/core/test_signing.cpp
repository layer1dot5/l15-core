...
TEST_CASE("KeyLookup")
{
    ...
    std::clog << "Derived addr: " << derived.GetP2TRAddress(Bech32(BTC, TESTNET)) << std::endl;
...
TEST_CASE("Derive")
{
    ...
    if (keypath.starts_with("m/86")) {
        core::SchnorrKeyPair derived_key(sk);
        std::clog << "+++seckey: " << hex(derived_key.GetPrivKey()) << std::endl;
        derived_key.AddTapTweak();
        std::clog << "tweaked seckey: " << hex(derived_key.GetPrivKey()) << std::endl;
        KeyPair keypair(derived_key.GetPrivKey());
...
//    const std::string signature_hex = "485e41ef0e4e7c33be3010275a4fc1852dda058e2a55179374a80ca7abddb5131146cd6e2838ed016fd0dd2f28aeacdbe53238af4e402fc051cc02fa83be65b4";
    ...
    CHECK(pk.verify(l15::core::SchnorrKeyPair::GetStaticSecp256k1Context(), unhex<signature>(signature_hex), uint256S(challenge_hex)));
}
...
TEST_CASE("MakeSignature")
{
    ...
    core::SchnorrKeyPair keypair(sk);

    signature sig = keypair.SignSchnorr(uint256S(challenge_hex));

    xonly_pubkey pk = keypair.GetPubKey();

    CHECK(pk.verify(l15::core::SchnorrKeyPair::GetStaticSecp256k1Context(), sig, uint256S(challenge_hex)));
}
...
TEST_CASE("PubKeyDerive")
{
    ...
    std::clog << "tweaked seckey: " << hex(keypair0.GetPrivKey()) << std::endl;
    KeyPair trkeypair0(keypair0.GetPrivKey());
    std::clog << "btc addr 0: " << trkeypair0.GetP2TRAddress(Bech32(BTC, TESTNET)) << std::endl;
