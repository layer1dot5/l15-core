...
void StartNode(NodeChainMode mode, ExecHelper& node_exec, const CLI::App& options)
{
    ...
    if(mode == NodeChainMode::MODE_REGTEST) node_exec.Arguments().emplace_back("-regtest");
    else if(mode == NodeChainMode::MODE_TESTNET) node_exec.Arguments().emplace_back("-testnet");
...
void StopNode(NodeChainMode mode, ExecHelper& cli_exec, const CLI::App& options)
{
    ...
    if(mode == NodeChainMode::MODE_REGTEST) cli_exec.Arguments().emplace_back("-regtest");
    else if(mode == NodeChainMode::MODE_TESTNET) cli_exec.Arguments().emplace_back("-testnet");
