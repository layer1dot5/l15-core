    ...
    void Serialize(auto& s) const {
        ...
        if (!final_script_sig.has_value() && !final_script_witness.has_value()) {
            ...
            for (const auto& [hash, preimage] : ripemd160_preimages) {
                SerializeToVector(s, CompactSizeWriter(PSBT_IN_RIPEMD160), hash);
                s << preimage;
            }

            // Write any sha256 preimage
            for (const auto& [hash, preimage] : sha256_preimages) {
                SerializeToVector(s, CompactSizeWriter(PSBT_IN_SHA256), hash);
                s << preimage;
            }

            // Write any hash160 preimage
            for (const auto& [hash, preimage] : hash160_preimages) {
                SerializeToVector(s, CompactSizeWriter(PSBT_IN_HASH160), hash);
                s << preimage;
            }

            // Write any hash256 preimage
            for (const auto& [hash, preimage] : hash256_preimages) {
                SerializeToVector(s, CompactSizeWriter(PSBT_IN_HASH256), hash);
                s << preimage;
            }
            ...
            for (const auto& [pubkey_leaf, sig] : m_tap_script_sigs) {
                ...
                SerializeToVector(s, PSBT_IN_TAP_SCRIPT_SIG, xonly.get_vector(), leaf_hash);
                s << sig;
            }

            // Write taproot leaf scripts
            for (const auto& [script, control_block] : m_tap_scripts) {
                SerializeToVector(s, PSBT_IN_TAP_LEAF_SCRIPT, control_block);
                bytevector value_v(script.begin(), script.end());
                value_v.push_back((uint8_t)0x0C0);
                s << value_v;
            }
            ...
            for (const auto& [xonly, leaf_origin] : m_tap_bip32_paths) {
                ...
                SerializeToVector(s, PSBT_IN_TAP_BIP32_DERIVATION, xonly.get_vector());
                bytevector value;
...
struct PSBTOutput
{
    ...
    void Serialize(auto& s) const {
        ...
        for (const auto& [xonly, leaf] : m_tap_bip32_paths) {
            ...
            SerializeToVector(s, PSBT_OUT_TAP_BIP32_DERIVATION, xonly.get_vector());
            bytevector value;
            VectorWriter s_value{value, 0};
            s_value << leaf_hashes;
            SerializeKeyOrigin(s_value, origin);
        ...
        while(!s.empty()) {
            ...
            switch(type) {
                ...
                case PSBT_OUT_TAP_TREE:
                {
                    ...
                    //     throw TransactionError("Output Taproot tree is malformed");
                    // }
                    break;
                }
...
struct PartiallySignedTransaction
{
    ...
    //std::map<KeyOriginInfo, std::set<ext_pubkey>> m_xpubs;
    ...
    void Unserialize(auto raw_data) {
        ...
        if (inputs.size() != tx->vin.size())
            throw TransactionError("Inputs provided does not match the number of inputs in transaction.");
        ...
        if (outputs.size() != tx->vout.size())
            throw TransactionError("Outputs provided does not match the number of outputs in transaction.");
    }
};

// template <typename Stream>
// Stream& operator<< (Stream& s, const PartiallySignedTransaction& psbt)
// {
//     psbt.Serialize(s);
//     return s;
// }
